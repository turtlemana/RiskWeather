...
const Detail=({riskData, priceData,assetData,corrData,predictData,allAssetData,jsonData,indexData})=>{
    ...
    const corr1=useMemo(()=>{return corrCardData.slice(0,4)},[corrData])
    const corr2=useMemo(()=>{return corrCardData.slice(1,5)},[corrData])
    const corr3=useMemo(()=>{return corrCardData.slice(2,6)},[corrData])
    const corr4=useMemo(()=>{return corrCardData.slice(3,7)},[corrData])
    const corr5=useMemo(()=>{return corrCardData.slice(4,8)},[corrData])
    const corr6=useMemo(()=>{return corrCardData.slice(5,9)},[corrData])
    const corr7=useMemo(()=>{return corrCardData.slice(6,10)},[corrData])
...
return tmpCorr10},[corrData])
    ...
    useEffect(()=>{setCorrCardSlide(corrMain); ;setCurrentSlide(0); setIsActive("Chart")},[allAssetData,corrData])
                        ...
                        <div className={`flex justify-between flex-col 2xl:flex-row`}>
                            <div className={`flex p-4 flex-col xl:flex-row`}>
                                ...
                                <div className={`flex flex-col justify-between min-[1280px]:ml-10 px-5`}>
                                    <h2 className={` text-4xl font-bold text-gray-600 mb-4 max-[450px]:text-[30px] max-[1279px]:text-center`}>{assetData[0].weather.replace("_"," ")}</h2>
                                    <div className={`mb-4`}>
                                        <div className={`flex items-center gap-3 mb-4`}>
                                            <h3 className={`text-gray-500 p-1.5 px-6 bg-gray-100 rounded-full max-[450px]:text-[12px]`}>PRICE</h3>
                                            <p className={`text-4 max-[450px]:text-[18px]`}>${assetData[0].price.toFixed(2)}</p>
                ...
                <div className={`tab-content`}>
                    <div className={`flex flex-col 2xl:flex-row gap-5 mb-16`}>
                        <div className={`container-line flex-1 max-[1280px]:chart-container-line`}>
                            ...
                            : infoData[0]? (<div>
                                <h1 className="text-4 font-medium mb-2">{infoData[0]?.name}</h1>
                                {/* <p className="text-3">{infoData[0]?.ticker}</p> */}
                                <p className="text-6 font-light">{infoData[0]?.info}</p>
                                </div>) :(<p className="text-2">We are going to add an information as soon as possible</p>)
                            }
...
export async function getServerSideProps(context) {
    ...
    const [res,res2,res3,res4,res5,res6,res7,res8] = await Promise.all([
    ...
    const [riskData, priceData,assetData,corrData,predictData,allAssetData,jsonData,indexData]=await Promise.all([
        res.json(),
        res2.json(),
        res3.json(),
        res4.json(),
    ...
    return { props: {riskData,priceData,assetData,corrData,predictData,allAssetData,jsonData,indexData} };
  }
