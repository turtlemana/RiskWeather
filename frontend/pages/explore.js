...
const Explore=({mainCard,allAssets,cryptoExchange,miniData})=>{
...
return mainData},[mainCard,allAssets])
    ...
    const [tableAssets,setTableAssets]=useState(chartData)

    
    useEffect(()=>{
        if(assetSelect=='ALL'){
            const tableDt=chartData.filter((asset)=>asset.cat=="Crypto"||asset.cat=="Index"||asset.cat=="Stock")
            setTableAssets(tableDt)

        }
        if(assetSelect=="Crypto"){
            const tableDt=chartData.filter((asset)=>asset.cat=="Crypto")
            setTableAssets(tableDt)
        }
        if(assetSelect=="Index"){
            const tableDt=chartData.filter((asset)=>asset.cat=="Index")
            setTableAssets(tableDt)
        }
        if(assetSelect=="Stock"){
            const tableDt=chartData.filter((asset)=>asset.cat=="Stock")
            setTableAssets(tableDt)
        }
    },[chartData,assetSelect])
    ...
    const majorIndex=useMemo(()=>{
        const defaultIndex=chartData.filter((idx) =>
    ...
    const majorStock=useMemo(()=>{
        const defaultStock=chartData.filter((stock) =>
...
export async function getServerSideProps() {
    ...
    const [res,res2,res3,res4] = await Promise.all([
        ...
        // fetch('https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest?limit=200&price_min=3000', {
        //     headers: {
    ...
    const [mainCard,allAssets,cryptoExchange,miniData,coinBaseData]=await Promise.all([
        res.json(),
        res2.json(),
        res3.json(),
        res4.json(),
    ...
    return { props: { mainCard,allAssets,cryptoExchange,miniData } };
  }
